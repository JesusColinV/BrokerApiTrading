...
alice = None
socket_opened = False
entry_time = datetime.time(9,30) #Entry time for placing the straddle or strangle
exit_time = datetime.time(15,10) #Exit time for closing the straddle or strangle


symbol = 'NIFTY' # symbol
fut_expiry_date = datetime.date(2021, 6, 24)  #Change to recent month
expire_on = '21JUN'  # Change this to recent month
strike_range = 0

livefeed = LiveFeedType.MARKET_DATA
no_of_lots = 1
product_type = ProductType.Intraday
datecalc = datetime.date(2021, 6 , 24)  # Change to recent month
step = 50  # Step size for management
...
def manage_strangle_or_straddle():
	global ce_strike, pe_strike, ltp, alice

	print('\n!! Management !!!')
	print(datetime.datetime.now().time())

	midpoint = int((ce_strike+pe_strike)/2)
	...
	if abs(curr_ltp - midpoint) > step:
		if (curr_ltp - midpoint) > step:
			print(f"\nLTP is at {curr_ltp - midpoint} far from midpoint: Upside Violation")
		elif (midpoint - curr_ltp) > step:
			print(f"\nLTP is at {curr_ltp - midpoint} far from midpoint: Downside Violation")

		print("\nClosing Old Sell CE and PE Position")
		close_strangle_or_straddle()

		get_prices()
		print("\nPlacing new Order near LTP")
		place_strangle_or_straddle()

	else:
		print(f"\nLTP is at {curr_ltp - midpoint} far from midpoint: within -{step} to +{step}")


def close_strangle_or_straddle():
	global ce_strike, pe_strike
	open_positions()

	check_for_valid_strangle_or_straddle()

	close_get_ce_curr_price_spread(ce_strike)
	close_get_pe_curr_price_spread(pe_strike)

	print("closed strangle or straddle")

def close_get_ce_curr_price_spread(ce_strike):
	global alice, ltp, datecalc
	...
	Buy_ce_option(n_call)

	alice.unsubscribe(n_call, livefeed)

def close_get_pe_curr_price_spread(pe_strike):
	global alice, ltp, datecalc
...
def Buy_ce_option(n_call):
	...
	try:
		if order['status'] == 'success':
			print(f"\nCE Buy Order at {ce_strike} placed {no_of_lots} lots at price = {ltp}")
	except Exception as e:
		print("Buy CE Order placing status failed =>", e)

def Buy_pe_option(n_put):
	global ce_strike, pe_strike
	quantity = no_of_lots * int(n_put[5])
	...
	try:
		if order['status'] == 'success':
			print(f"\nPE Buy Order at {pe_strike} placed {no_of_lots} lots at price = {ltp}")
	except Exception as e:
		print("Buy PE Order placing status failed =>", e)
...
def main():
	...
	while True:
		...
		if (datetime.datetime.now().second % 10 == 0):
			...
			elif order_placed == True:
				if datetime.datetime.now().time() < exit_time:

					try:
						manage_strangle_or_straddle()
					except Exception as e:
						print('error in management of trades =>', e)
						sleep(1)
				else:

					print("Times up. Exit all Sell orders")
					close_strangle_or_straddle()
					print("\nClosing the program\n")
					sleep(1)
